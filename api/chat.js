...
const buildSystem = (lang, productsCtx) => {
  if (lang === 'en') {
    return `You are a customer service assistant for ĐẠI THÀNH CÔNG (DTC), a network of 7 authorized motorcycle stores in Bình Dương province, Vietnam. Brands: Honda, Yamaha, SYM, Suzuki, Piaggio, VinFast, Kawasaki.

KEY INFO:
- Hotline: 1900 6789 (07:30 — 21:00 daily)
- 7 branches: Thủ Dầu Một (HQ), Thuận An, Dĩ An, Tân Uyên, Bến Cát, Phú Giáo, Bàu Bàng
- Email: info@daithanhcong.vn · Zalo OA: @daithanhcong.vn
...
${productsCtx || '(unavailable, suggest hotline)'}`;
  ...
  return `Bạn là trợ lý tư vấn của ĐẠI THÀNH CÔNG (ĐTC) — hệ thống 7 cửa hàng xe máy chính hãng tại Bình Dương. Các hãng phân phối: Honda, Yamaha, SYM, Suzuki, Piaggio, VinFast, Kawasaki.

THÔNG TIN CỐT LÕI:
- Hotline: 1900 6789 (07:30 — 21:00 hàng ngày)
...
PHONG CÁCH:
- Xưng "em", gọi khách "anh/chị", thân thiện chuyên nghiệp
...
export default async function handler(req, res) {
  ...
  try {
    const productsCtx = await loadProductsContext();
    const messages = [
    ...
    const aiRes = await fetch('https://api.anthropic.com/v1/messages', {
      ...
      body: JSON.stringify({
        ...
        max_tokens: 400,
        system: buildSystem(lang, productsCtx),
        messages
      })
    });
